defmodule FoxWeb.UserAuthTest do
  ...
  @remember_me_cookie "_fox_web_user_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, FoxWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      FoxWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "require_authenticated_user/2" do
    test "redirects if user is not authenticated", %{conn: conn} do
      ...
      assert conn.halted
      assert redirected_to(conn) == Routes.live_path(conn, UserSessionLive)
